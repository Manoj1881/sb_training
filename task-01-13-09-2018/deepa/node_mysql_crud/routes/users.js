...
app.post('/add', function (req, res, next) {
    ...
    req.assert('password', 'A valid password').len(8, 100)
    var errors = req.validationErrors()

    if (!errors) {   //No errors were found.  Passed Validation!

        var name = req.sanitize('name').escape().trim()
        var age = req.sanitize('age').escape().trim()
        ...
        req.getConnection(function (error, conn) {
            bcrypt.hash(password, saltRounds, function (err, hash) {
                ...
                conn.query('INSERT INTO users (name, age, email, password) values (?, ?, ?, ?)', [name, age, email, hash], function (err, result) {
